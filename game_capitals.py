import random
from telebot import types

# نظام النقاط المرتبط بالـ Volume
try:
    from db_manager import get_user, update_user
except:
    def get_user(uid): return {"balance": 1000}
    def update_user(uid, k, v): pass

def register_handlers(bot):
    
    # قاعدة البيانات (الدولة: العاصمة)
    CAPITALS_DB = {
        "العراق": "بغداد", "السعودية": "الرياض", "مصر": "القاهرة", "سوريا": "دمشق",
        "الأردن": "عمان", "فلسطين": "القدس", "لبنان": "بيروت", "الكويت": "الكويت",
        "قطر": "الدوحة", "الإمارات": "أبوظبي", "المغرب": "الرباط", "الجزائر": "الجزائر",
        "تونس": "تونس", "ليبيا": "طرابلس", "السودان": "الخرطوم", "تركيا": "أنقرة",
        "إيران": "طهران", "أفغانستان": "كابل", "فرنسا": "باريس", "ألمانيا": "برلين",
        "إيطاليا": "روما", "إسبانيا": "مدريد", "بريطانيا": "لندن", "روسيا": "موسكو",
        "الصين": "بكين", "اليابان": "طوكيو", "كوريا الجنوبية": "سيول", "الهند": "نيودلهي",
        "باكستان": "إسلام آباد", "الولايات المتحدة": "واشنطن", "كندا": "أوتاوا", "البرازيل": "برازيليا",
        "الأرجنتين": "بوينس آيرس", "المكسيك": "مكسيكو سيتي", "أستراليا": "كانبيرا", "نيوزيلندا": "ويلينغتون",
        "جنوب أفريقيا": "بريتوريا", "نيجيريا": "أبوجا", "كينيا": "نيروبي", "إثيوبيا": "أديس أبابا",
        "السويد": "ستوكهولم", "النرويج": "أوسلو", "فنلندا": "هلسنكي", "سويسرا": "برن",
        "النمسا": "فيينا", "بلجيكا": "بروكسل", "اليونان": "أثينا", "البرتغال": "لشبونة",
        "تشيلي": "سانتياغو", "بيرو": "ليما"
    }

    active_capitals = {}

    @bot.message_handler(func=lambda m: m.text == "عواصم")
    def start_capital_game(m):
        chat_id = m.chat.id
        country, capital = random.choice(list(CAPITALS_DB.items()))
        
        active_capitals[chat_id] = capital
        
        text = (
            "┏━━━━━━━ ● ━━━━━━━┓\n"
            "         ⌯ تـحـدي الـعـواصـم ⌯\n"
            "┗━━━━━━━ ● ━━━━━━━┛\n\n"
            "⚡️ مـن الأسـرع ؟\n"
            f"🌍 مـا هـي عـاصـمـة : [ **{country}** ]\n\n"
            "💰 الـجـائزة : 40 نـقـطـة\n"
            "⚠️ أرسل الإجابة الآن في الشات!"
        )
        bot.send_message(chat_id, text, parse_mode="Markdown")

    @bot.message_handler(func=lambda m: m.chat.id in active_capitals)
    def check_capital(m):
        chat_id = m.chat.id
        correct_answer = active_capitals[chat_id]
        
        # التأكد من مطابقة الإجابة (تجاهل الهمزات البسيطة لسهولة اللعب)
        user_answer = m.text.strip().replace("أ", "ا").replace("إ", "ا")
        normalized_correct = correct_answer.replace("أ", "ا").replace("إ", "ا")

        if user_answer == normalized_correct:
            uid = m.from_user.id
            user_bal = get_user(uid).get("balance", 0)
            update_user(uid, "balance", user_bal + 40)
            
            win_text = (
                "⌯ غـزو جـغـرافـي نـاجـح ⌯\n"
                "━━━━━━━━━━━━━━\n"
                f"👤 الـفـائـز : {m.from_user.first_name}\n"
                f"✅ الإجـابـة : {correct_answer}\n"
                "💰 الـجـوائـز : +40 نـقـطـة"
            )
            bot.reply_to(m, win_text)
            del active_capitals[chat_id]
