import random
import time
from telebot import types
import db_manager # الربط بالخزنة الملكية

def register_handlers(bot):
    
    # القائمة الملكية - كما هي بدون تغيير
    KEYBOARD_WORDS = [
        "قسطنطينية", "فسيكفيكهم الله", "استسقيناكموها", "فأسقيناكموه", "ليستخلفنهم",
        "مستضعفون", "سيجعل لهم الرحمن وُدًّا", "أفغيرَ الله تأمروني", "فأنجيناه وأهله", "المتشابهات",
        "أتحاجّوني في الله", "فتماروا في أمرهم", "سيهزم الجمع ويولّون الدبر", "استكبروا واستعتوا", "فأنساهم أنفسهم",
        "مستكبرين به سامرًا", "ليستخلفنهم في الأرض", "أفتطمعون أن يؤمنوا لكم", "فدمدم عليهم ربهم", "فجعلكم خلائف",
        "الإسكندرية", "نيقولا تسلا", "الإمبراطورية", "الاستعمارية", "الميتافيزيقيا",
        "اللامركزية", "السيكولوجيا", "الأنثروبولوجيا", "الإيديولوجيات", "الاستشراق",
        "متناقضات", "استحالة", "استذكار", "اللاوعي", "فوق إدراكي",
        "تشظّي", "انكسارات", "تموضع", "تماهٍ", "الوجودية",
        "استبطان", "ماورائيات", "استقراء", "استنباط", "تداخلات",
        "اللامحدودية", "تشابكات", "احتمالات", "فوضوية", "عبثيّة"
    ]

    active_challenges = {}

    @bot.message_handler(func=lambda m: m.text == "كيبورد")
    def start_keyboard_game(m):
        chat_id = m.chat.id
        word = random.choice(KEYBOARD_WORDS)
        
        active_challenges[chat_id] = {
            "word": word,
            "winner": None,
            "start_time": time.time()
        }
        
        text = (
            "┏━━━━━━━ ● ━━━━━━━┓\n"
            "         ⌯ تـحـدي الـكـيـبـورد ⌯\n"
            "┗━━━━━━━ ● ━━━━━━━┛\n\n"
            "⚡️ أسرع واحد يكتب الكلمة التالية:\n"
            f"✨ [ `{word}` ] ✨\n\n"
            "💰 الـجـائـزة : 100 ذهبة\n"
        )
        
        bot.send_message(chat_id, text, parse_mode="Markdown")

    @bot.message_handler(func=lambda m: m.chat.id in active_challenges)
    def check_typing_speed(m):
        chat_id = m.chat.id
        challenge = active_challenges[chat_id]
        
        # التأكد من صحة الكلمة
        if m.text.strip() == challenge["word"]:
            # الفائز الأول
            if not challenge["winner"]:
                challenge["winner"] = m.from_user.id
                elapsed_time = round(time.time() - challenge["start_time"], 2)
                
                # تحديث الذهب في الخزنة الحقيقية
                db_manager.update_user_gold(m.from_user.id, 100)
                
                win_text = (
                    "⌯ وحـش الـكـيـبـورد وصـل ⌯\n"
                    "━━━━━━━━━━━━━━\n"
                    f"👤 الـفـائـز : {m.from_user.first_name}\n"
                    f"⏱️ الـوقـت : {elapsed_time} ثـانـيـة\n"
                    "💰 الـجـوائـز : +100 ذهـبـة"
                )
                bot.reply_to(m, win_text)
            
            # المتأخرون (قصف جبهات)
            else:
                slow_replies = [
                    "بدري يا بطل! الفوز لغيرك، خليك أسرع المرة الجاية 🐢",
                    "يا بطيء! الناس فازت واعتزلت وأنت توك تكتب؟ 😂",
                    "يضعيف! أصابعك يبيلها تشحيم 🔧",
                    "مسوي فيها سريع؟ فاتك القطار يا كابتن 🚂",
                    "شكلك تكتب بصباع واحد.. صح؟ 🤡"
                ]
                bot.reply_to(m, random.choice(slow_replies))
                # حذف التحدي بعد أول رد بطيء
                del active_challenges[chat_id]
