import random
from telebot import types

# نظام النقاط المرتبط بالـ Volume
try:
    from db_manager import get_user, update_user
except:
    def get_user(uid): return {"balance": 0}
    def update_user(uid, k, v): pass

def register_handlers(bot):
    
    # مخزن الكلمات الملكي (150 كلمة)
    WORDS_DB = [
        "سلمون", "كمبيوتر", "مدرسة", "تليجرام", "أسد", "فلسطين", "العراق", "مصر", "برتقال", "سيارة",
        "طائرة", "كتاب", "تلفاز", "رمضان", "قهوة", "موز", "تفاح", "عنب", "مانجو", "خوخ",
        "بطيخ", "فراولة", "كرز", "ليمون", "جوافة", "اناناس", "مشمش", "تين", "زيتون", "رمان",
        "سوريا", "لبنان", "الاردن", "الكويت", "البحرين", "قطر", "الامارات", "عمان", "اليمن", "المغرب",
        "تونس", "الجزائر", "ليبيا", "السودان", "الصومال", "بغداد", "القدس", "دمشق", "بيروت", "عمان",
        "المنامة", "الدوحة", "مسقط", "صنعاء", "الرباط", "تونس", "الجزائر", "طرابلس", "الخرطوم", "نيروبي",
        "لندن", "باريس", "روما", "مدريد", "برلين", "طوكيو", "بكين", "سيول", "موسكو", "واشنطن",
        "نمر", "فهد", "ذئب", "ثعلب", "دب", "زرافة", "فيل", "حصان", "جمل", "بقرة",
        "خروف", "ماعز", "ارنب", "سنجاب", "قرد", "تمساح", "ثعبان", "سلحفاة", "ضفدع", "عقرب",
        "نسر", "صقر", "بومة", "حمامة", "عصفور", "بطة", "دجاجة", "طاووس", "بطريق", "نعامة",
        "طاولة", "كرسي", "سرير", "دولاب", "نافذة", "باب", "مفتاح", "قلم", "دفتر", "مسطرة",
        "ممحاة", "حقيبة", "ساعة", "هاتف", "شاحن", "سماعة", "كاميرا", "مذياع", "مصباح", "ثلاجة",
        "فرن", "ملعقة", "شوكة", "سكين", "طبق", "كوب", "ابريق", "منشفة", "صابون", "مرآة",
        "ملابس", "قميص", "بنطال", "فستان", "حذاء", "جورب", "قبعة", "نظارة", "خاتم", "سوار",
        "ذهب", "فضة", "حديد", "نحاس", "الماس", "بحر", "نهر", "جبل", "غابة", "صحراء",
        "قمر", "شمس", "نجوم", "كوكب", "سماء", "سحاب", "مطر", "ثلج", "رعد", "برق"
    ]

    active_games = {}

    @bot.message_handler(func=lambda m: m.text == "فكك")
    def start_game(m):
        chat_id = m.chat.id
        word = random.choice(WORDS_DB)
        
        # تحويل الكلمة إلى حروف مرتبة وبينها مسافات "ا - س - د"
        shuffled = " - ".join(list(word))
        
        active_games[chat_id] = word
        
        text = (
            "┏━━━━━━━ ● ━━━━━━━┓\n"
            "         ⌯ تـحـدي الـتـرتـيـب ⌯\n"
            "┗━━━━━━━ ● ━━━━━━━┛\n\n"
            f"  » جـمّـع الـكلمة الـتـالـيـة :\n"
            f"📦 [ {shuffled} ]\n\n"
            "⚠️ أرسل الكلمة الصحيحة الآن\n"
            "💰 الـجـائـزة : 20 نـقـطـة"
        )
        bot.send_message(chat_id, text)

    @bot.message_handler(func=lambda m: m.chat.id in active_games)
    def check_answer(m):
        chat_id = m.chat.id
        # التأكد من مطابقة الحل (حذف المسافات من إجابة اللاعب إن وجدت)
        if m.text.strip() == active_games[chat_id]:
            uid = m.from_user.id
            user_bal = get_user(uid).get("balance", 0)
            update_user(uid, "balance", user_bal + 20)
            
            win_text = (
                "⌯ تـم الـتـحـقـق مـن الإجـابـة ⌯\n"
                "━━━━━━━━━━━━━━\n"
                f"👤 الـفـائـز : {m.from_user.first_name}\n"
                "✅ الإجـابـة : صـحـيـحـة (كـفـو)\n"
                "💰 الـجـوائـز : +20 نـقـطـة"
            )
            bot.reply_to(m, win_text)
            del active_games[chat_id]
